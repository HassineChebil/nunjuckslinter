...
export const createLintingRules = (linter: LinterInterface): Record<string, (content: string, filename: string) => void> => ({
    checkBlockStructure: (content: string, filename: string): void => {
        ...
        const handleComments = (starts: Array<RegExpMatchArray>, ends: Array<RegExpMatchArray>): boolean => {
        ...
        const validateBlockSyntax = (pattern: BlockPattern, match: RegExpMatchArray, index: number): void => {
            if (pattern.type === 'macro' && match[2]) {
                ...
                params.forEach(param => {
                    if (param && !param.match(/^[a-zA-Z_][a-zA-Z0-9_]*$/)) {
                        linter.addError(filename, index + 1, `Invalid macro parameter name: ${param}`);
                    }
                });
            }
        };
        ...
        const checkBlockEndings = (line: string, index: number): void => {
            ...
            for (const [name, pattern] of Object.entries(BLOCK_PATTERNS)) {
                ...
                if (blockStack.length === 0) {
                    linter.addError(filename, index + 1, `Unexpected end${pattern.type} without matching opening tag`);
                    // hasError = true;
                    return;
                }
        ...
        if (!inMultilineComment && blockStack.length > 0) {
            // Report only the first unclosed block
            // const lastBlock = blockStack[blockStack.length - 1];
            // linter.addError(
            //     filename,
            //     lastBlock.line,
            //     `Unclosed ${lastBlock.type}${lastBlock.type === 'block' ? ` "${lastBlock.name}"` : ''} at line ${lastBlock.line}`
            // );
            ...
            blockStack.forEach(block => {
                linter.addError(
                    filename,
                    block.line,
                    `Unclosed ${block.type}${block.type === 'block' ? ` "${block.name}"` : ''} at line ${block.line}`
                );
            });
        }
    },
    checkSyntaxBalance: (content: string, filename: string): void => {
        ...
        lines.forEach((line, index) => {
            ...
            const braceMatches = [...lineWithoutComments.matchAll(SYNTAX_PATTERNS.braces.open)].length;
            const braceCloses = [...lineWithoutComments.matchAll(SYNTAX_PATTERNS.braces.close)].length;
            ...
            for (let i = 0; i < braceCloses; i++) {
                if (state.openBraces.length === 0) {
                    linter.addError(filename, index + 1, `Unexpected closing braces '}}' without matching opening braces`);
                    return;
                }
            ...
            // Check tags balance
            const tagMatches = [...lineWithoutComments.matchAll(SYNTAX_PATTERNS.tags.open)].length;
            const tagCloses = [...lineWithoutComments.matchAll(SYNTAX_PATTERNS.tags.close)].length;
            ...
            for (let i = 0; i < tagCloses; i++) {
                if (state.openTags.length === 0) {
                    linter.addError(filename, index + 1, `Unexpected closing tag '%}' without matching opening tag`);
                    return;
                }
            ...
            const commentMatches = [...lineWithoutComments.matchAll(SYNTAX_PATTERNS.comments.open)].length;
            const commentCloses = [...lineWithoutComments.matchAll(SYNTAX_PATTERNS.comments.close)].length;
        ...
        if (!state.inMultilineComment) {
            if (state.openBraces.length > 0) {
                const totalUnclosed = state.openBraces.reduce((sum, pos) => sum + pos.count, 0);
                linter.addError(
                    filename, 
            ...
            if (state.openTags.length > 0) {
                const totalUnclosed = state.openTags.reduce((sum, pos) => sum + pos.count, 0);
                linter.addError(
                    filename, 
            ...
            if (state.openComments.length > 0) {
                const totalUnclosed = state.openComments.reduce((sum, pos) => sum + pos.count, 0);
                linter.addError(
                    filename, 
